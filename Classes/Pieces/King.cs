            ...
            if (!(board.kingThatHaveMoved.Contains(this)))
            {
                ...
                if (!tilesAttacked.Contains(this.location))
                {
                    ...
                    if (piece[rookLongLocation] is Rook && !(board.rooksThatHaveMoved.Contains(piece[rookLongLocation])))
                    {
                        ...
                        {
                            Move move = new Move(currentLocation, currentLocation - 2, this, MoveTag.castleLong, piece[rookLongLocation]);
                            validMoves.Add(move);
                        }
                    ...
                    if (piece[rookShortLocation] is Rook && !board.rooksThatHaveMoved.Contains(piece[rookShortLocation]))
                    {
                        ...
                        if (inBetweenAttacked == false && squaresBetweenNotEmpty == false)
                        {
                            Move move = new Move(currentLocation, currentLocation + 2, this, MoveTag.castleShort, piece[rookShortLocation]);
                            validMoves.Add(move);
                        }
                    }
